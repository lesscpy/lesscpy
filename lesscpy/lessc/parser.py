...
class LessParser:
        ...
        if not file:
            ...
            if hasattr(file, "name"):
                if filename is not None:
                    raise AssertionError("names of file and filename are in conflict")
                filename = file.name
            else:
                filename = "(stream)"
    ...
    def p_statement_import(self, p):
        ...
        if isinstance(p[3], str):
            ...
            p[3] = p[3].parse(
                self.scope
            )  # Store it as string, Statement.fmt expects it.
    ...
    def p_property_decl(self, p):
        ...
        """
        length = len(p)
        p[0] = Property(list(p)[1:-1], p.lineno(length - 1))
