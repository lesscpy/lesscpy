...
from lesscpy.lessc import formatter, lexer, parser
import argparse
import copy
import errno
import glob
...
def ldirectory(inpath, outpath, args, scope):
    ...
    if args.recurse:
        [
            ldirectory(
                os.path.join(inpath, name), os.path.join(outpath, name), args, scope
            )
            for name in os.listdir(inpath)
            if os.path.isdir(os.path.join(inpath, name)) and not name.startswith(".") and not name == outpath
        ]
...
def run():
    ...
    aparse.add_argument("-v", "--version", action="version", version=VERSION_STR)
    aparse.add_argument(
        "-I",
        "--include",
        action="store",
    ...
    fgroup.add_argument(
        "-x", "--minify", action="store_true", default=False, help="Minify output"
    )
    fgroup.add_argument(
        "-X",
        "--xminify",
    ...
    group.add_argument(
        "-S", "--scopemap", action="store_true", default=False, help="Scopemap"
    )
    group.add_argument(
        "-L",
        "--lex-only",
    ...
    group.add_argument(
        "-N", "--no-css", action="store_true", default=False, help="No css output"
    )
